...
class MinimaxBloomsPlayer(BloomsPlayer):
    ...
    def __heuristic(self, state: BloomsState):
        grid = state.get_board_01()
        longest = 0
    ...
    def minimax(self, state: BloomsState, depth: int, alpha: int = -sys.maxsize, beta: int = sys.maxsize,
                is_initial_node: bool = True):
        # first we check if we are in a terminal node (victory, draw or loose)
        if state.is_finished():
            return {
                BloomsResult.WIN: 4,
                BloomsResult.LOOSE: -4,
                BloomsResult.DRAW: 0
    ...
    def get_action(self, state: BloomsState):
        return BloomsAction(self.minimax(state, 5))
